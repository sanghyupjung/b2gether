    ...
    Days = (firstDayFormat, weekIndex) => {
        ...
        for(let i = 0; i < 7; i++) {
            ...
            _days.push({
                ...
                todolistFormat: Day.format("YYYYMMDD"),
                getDay: Day.format('D'),
                isHolyDay: false,
                weekIndex
            });
            ...
            return (
                <div className={"RCA-calendar-day " + className} key={`RCA-${dayInfo.weekIndex}-${i}-day`} onClick={() => fn(dayInfo.yearMonthDayFormat)}>
                    <label className="RCA-calendar-day-label" onClick={() => {
                        // console.log(dayInfo.todolistFormat);
                        this.props.history.push("/makeTodolist/" + dayInfo.todolistFormat);
                    }}>
                        {dayInfo.getDay}
                    </label>
                    {/* <div>todolist</div> */}
...
const WeekWithRouter = withRouter(Week);
...
class ShowCalendar extends Component {
    Weeks = (monthYear, selected, clickFn) => {
        ...
        for(let i = 0; i < 6; i++) {
            _Weeks.push((
                <WeekWithRouter key={`RCA-calendar-week-${i}`}
                weekIndex={i}
